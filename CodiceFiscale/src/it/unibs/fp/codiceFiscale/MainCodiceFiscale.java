...
public class MainCodiceFiscale {
	...
	public static void main(String[] args) throws XMLStreamException {
		//Lettura del file input persone
		...
		while(xmlcf.hasNext()) {
			...
			case XMLStreamConstants.CHARACTERS:
				if (xmlcf.getText().trim().length() > 0) //Aiuta a non inserire testo contenente solo spazio
					elencoCodiciFiscali.add(xmlcf.getText());
			}
			xmlcf.next();
		}
		//Scrittura del file xml
		ArrayList<String> codiciPresenti=new ArrayList<String>(); //ArrayList contenente i codici fiscali trovati
		int contaCodici=0;  //Utile per sapere il numero di codici spaiati
		int size=elencoCodiciFiscali.size();  //Utile per sapere il numero di codici spaiati
		XMLOutputFactory xmlof = null;
		XMLStreamWriter xmlout = null; //File output
		try {
		...
		//Salvataggio dei codici invalidi in un ArrayList
		ArrayList<String> elencoCodiciInvalidi=new ArrayList<String>();
		for(int i=0; i<elencoCodiciFiscali.size(); i++) {
			...
			if(!codiceAttuale.isValido()) {
				elencoCodiciInvalidi.add(elencoCodiciFiscali.get(i));
		...
		xmlout.writeStartElement("invalidi"); //Apre <invalidi>
		xmlout.writeAttribute("invalidi", "numero="+elencoCodiciInvalidi.size());
		for(int i=0; i<elencoCodiciInvalidi.size(); i++) {
				xmlout.writeStartElement("codice");
				xmlout.writeCharacters(elencoCodiciInvalidi.get(i));
				xmlout.writeEndElement();
		}
		xmlout.writeEndElement();
		xmlout.writeStartElement("spaiati"); //Apre <spaiati>
		xmlout.writeAttribute("spaiati", "numero="+(size-(contaCodici+elencoCodiciInvalidi.size()))); //Totale dei codici fiscali-(codici fiscali trovati+invalidi)
		boolean trovato;
		for(int i=0; i<elencoCodiciFiscali.size(); i++) {
			trovato=false;
