...
public class Persona {
	...
	private String generaCognome(String cognome) {
		...
		return cognomeCodice.toString().toUpperCase(); //.toUpperCase è stato inserito nel caso in cui il cognome sia stato scritto in minuscolo
	}
	...
	private String generaNome(String nome) {
		...
		//Se il numero di consonanti è minore o uguale a 3 esegue un tipo di algoritmo per ricavare il codice del nome
		...
		//Altrimenti utilizza un altro algoritmo
		for(int i=0; i<nome.length(); i++) {
			if(!isVocale(nome.charAt(i)) && nomeCodice.length()<MASSIMA_LUNGHEZZA_STRINGA) {
				contaConsonanti++;
	...
	private String generaGiorno(String str) {
		int giorno;
		try{
			giorno=Integer.parseInt(str.substring(str.length()-2, str.length()));
		}catch (Exception e) {
			giorno=0; //Al posto di dare errore salva giorno=0, che non rientra nei parametri di nessun mese, di conseguenza il codice fiscale non sarà corretto e quindi assente 
		}
	...
	private String generaComune(String str) throws XMLStreamException {
		...
		return ""; //return se il comune non viene trovato, codice assente
	}
